...
**Status:** Mechanism **(a) FALSIFIED on the real app** — see "Update
2026-07-18" below; do not ship it. The subsurface/offload alternative is
confirmed to work **only at integer display scale** (also below). No shell-side
fix currently exists for Nvidia + fractional scale. Motivated by DEVLOG §10–§11,
§16–§17.

### Update 2026-07-18 — hardware A/B on the real app (verified)

Both shell-side mechanisms were tested on the live app on the GTX 1060
(driver 580.159.03), GTK 4.22.4 / WebKitGTK 2.52.3, at the user's **170% scale**:

- **Mechanism (a), `CachedOverlay`, FAILS on the real WebView.** With
  `STREMIO_CACHED_OVERLAY=1` the UI renders **black** — yet a hover tooltip fires,
  proving the WebView is loaded and hit-testing, only its pixels never paint. CPU
  is low (~6% vs the default path's ~54% on the idle Board) *precisely because it
  is compositing a blank texture instead of the UI*. Root cause:
  `WidgetPaintable::current_image()` of the WebKitGTK widget **rasterizes empty**
  on Nvidia — WebKit presents the web content through a path the GSK rasterizer
  cannot capture (not necessarily a literal Wayland subsurface: at 170% offload
  no-ops, yet the capture is still empty). The ADR's premise ("a software
  `GskCairoNode` we can snapshot") does not hold on the live WebView. The
  `overlay_bench` "cached ≈ texture" result (below) is real but **misleading**: its
  child is a `GtkDrawingArea` that *does* draw through GTK snapshot, so it captures
  fine — the bench cannot exercise the subsurface path that defeats the real UI.
  This mechanism is a **dead end for a WebView overlay**; caching a widget snapshot
  cannot capture web content that renders to a subsurface. (The very reason
  `GtkGraphicsOffload` exists.)

- **Fractional scale is required for the subsurface/offload alternative to work.**
  Independently re-researched and confirmed in the installed stack: GTK's Wayland
  backend declines a subsurface attach when the destination rect is **non-integral
  in device pixels** (`scaled_rect_is_integral` → the "Non-integral device
  coordinates" decline in `gdk/wayland/gdksubsurface-wayland.c`, present in GTK
  **4.22.4** source *and* the shipped `libgtk-4.so` binary). At scale 1.7 laid-out
  rects are essentially never integral, so offload **silently no-ops** and falls
  back to in-scene CPU compositing (this is the real reason the earlier
  `wip-cpu-fix-experiments` stash showed "CPU unchanged" — not the cairo overlap,
  which only *lowers* the subsurface). The offload path is therefore
  **VIABLE ONLY AT INTEGER SCALE (100%/200%)**; NVIDIA driver 580 *does* export
  dmabuf (`EGL_MESA_image_dma_buf_export` present), so export is not the blocker —
  scale is. Also note GTK 4.16+ already hands `GtkGLArea` a dmabuf texture, so
  wrapping the existing video GLArea in `GtkGraphicsOffload` *is* the dmabuf-offload
  route — no `GdkDmabufTextureBuilder` rewrite would change the outcome.

**Net:** on Nvidia + fractional scale, neither mechanism here works, and
(re-researched 2026-07-18) **no runtime bump or upstream pull fixes it today:**

- **No shippable flatpak runtime carries a fix.** We already target
  `org.gnome.Platform` **50**, which *is* the current stack (GTK 4.22.4 +
  WebKitGTK 2.52.x); nightly only reaches GTK 4.23-dev / WebKit 2.53.x, neither
  with a relevant fix.
- **WebKitGTK GPU compositing on the Nvidia proprietary driver is upstream
  WONTFIX / unresolved** (Bugzilla #262607 WONTFIX 2024-09; #180739), so no
  WebKit version is expected to make the transparent UI a GPU texture node here.
- **The GTK fractional-scale offload gate is fundamental**, not a stray guard:
  `fractional-scale-v1` leaves subsurface position/size rounding undefined, so
  non-integral device rects can't be offloaded — no MR proposes to relax it. The
  offload path therefore works **only at integer scale (100%/200%)**.
- **`KWIN_USE_OVERLAYS` does not help** and is a red herring: the CPU composite
  happens *inside the GTK client* before any buffer reaches KWin; overlay planes
  need the video already on its own dmabuf subsurface (which fails at 170% for the
  same reason), and the feature is off-by-default due to Nvidia display freezes.
- **Upstream `Stremio/stremio-linux-shell` has shipped nothing relevant** (commits
  through v1.1.2, 2026-07-06 are build/dep chores) — nothing to cherry-pick.

The only genuine escape today is **integer display scale** (which enables the
offload path) or **waiting on upstream WebKit** to make GPU compositing work on
Nvidia proprietary. See `BENCHMARKS.md` for measured numbers. Everything below
this line is the superseded proposal, retained for the reasoning trail.
...
## ADR-0005 — `FreezeOverlay`: exclude the idle UI from the render scene

**Status:** Accepted (implemented). Supersedes ADR-0003's mechanism (a)
(`CachedOverlay`, FALSIFIED) and its rejected subsurface alternative. Motivated by
DEVLOG §18; full plan in `IMPLEMENTATION_PLAN.md`; measured in `BENCHMARKS.md`.

### Context

The per-frame CPU cost is GSK re-compositing the transparent, software
(on Nvidia) WebKit UI over the video every frame. Both "separate the layers"
fixes are dead: a texture cache of the UI renders black (`current_image()`
rasterizes the WebView empty), and a Wayland subsurface is declined at fractional
scale (fundamental to `fractional-scale-v1`). Fractional scale is a hard product
requirement, so integer-scale-only fixes are out.

### Decision

Remove the UI's render nodes from the scene **while the UI is invisible anyway**.
During playback the player controls auto-hide (stremio-web adds `overlayHidden` to
`.player-container`) and subtitles are mpv-rendered, so nothing visible is lost.

- **`FreezeOverlay`** (`src/app/freeze_overlay/`) wraps the WebKit UI; while
  `frozen`, its `snapshot()` early-returns → **zero render nodes** → GSK never
  touches the child → per-frame cost falls to the video-only floor, at any scale,
  on any driver. Proven by the falsified `CachedOverlay` A/B itself (dropping the
  WebView node took CPU 54%→6%). No texture cache ⇒ none of ADR-0003(a)'s
  invalidation/black-screen failure modes.
- The child is **not** unmapped (rejecting `set_visible(false)`): it keeps
  keyboard focus, receives input (GTK4 picking is geometry-based), and does not
  flip WebKit's Page Visibility — so unfreezing is instant and flicker-free.
- **Signal:** an injected `shell_ui` DOM observer (`ipc/preload.js`) reports
  player-chrome visibility, fail-visible + 5 s heartbeat (the UI is hosted
  `web.stremio.com`, so anchors are version-fragile; drift degrades to today's CPU
  behaviour, never to a hidden UI).
- **Policy:** `frozen = playing && !ui_visible && past-500ms-debounce`; capture-
  phase motion/key/scroll controllers unfreeze eagerly on any input.

### Consequences

**Positive** — works at every scale and on every driver; no dependency on a
WebKit-on-Nvidia or GTK-offload upstream fix; no UX change (subtitles via mpv,
controls reappear on interaction); `graphics_offload` kept inside for the
integer-scale case.

**Negative / risks** — depends on a stremio-web DOM class (`overlayHidden`);
mitigated by fail-visible + heartbeat + a live-verifiable selector. A persistent
web-drawn element animating over advancing video would defeat it (none known;
buffering keeps the overlay live and the video isn't advancing then). Contingency
B (self-composite via `webkit6::WebView::snapshot()`) documented in
`IMPLEMENTATION_PLAN.md` if that ever appears.

### Alternatives considered (rejected)

- **`CachedOverlay`** — renders black on the real WebView (ADR-0003 Update).
- **`set_visible(false)`** — unmaps the WebView, losing focus/input and stalling
  WebKit timers; UX regression.
- **Subsurface / `GtkGraphicsOffload` of the video** — integer-scale only.
